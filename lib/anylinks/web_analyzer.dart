...
class WebAnalyzer {
  ...
  static Future<InfoBase> _getInfo(String url, bool multimedia) async {
    final response = await requestUrl(url);
  ...
  static Future<Response> requestUrl(String url,
      {int count = 0, String cookie, Function callback}) async {
    Response res;
    ...
    final IOStreamedResponse stream =
      ...
      // Database.updateRoomData(gt.Get.put(UserController()).user.activeroom, {
      //   "pinnedurl": ""
      // });
    ...
    if (stream.statusCode == HttpStatus.movedTemporarily ||
      ...
      if (stream.isRedirect && count < 6) {
        ...
        count++;
        client.close();
        return requestUrl(url, count: count, cookie: cookie);
      }
      ...
      if (uri.host == "m.tb.cn") {
        ...
        if (match != null) {
          ...
          if (newUrl != null) {
            return requestUrl(newUrl, count: count, cookie: cookie);
          }
        }
      }
    }
